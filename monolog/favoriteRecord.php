...
//DBからレビューデータを取得
$ReviewData = getFavoriteAll($_SESSION['user_id'],$currentMinNum,$listSpan);
...
<body>
  ...
  ?>
  <!-- メインコンテンツ -->
  <div class="container-contents-wide record_list">
    <!--レビュー一覧表示-->
    <h2 class="title-form">お気に入り一覧</h2>
    <div class="wrap-review record_list">
      <?php
        if(!empty($ReviewData['data'])){
          foreach($ReviewData['data'] as $key => $val):
      ?>
      <a href="reviewDetail.php<?php echo '?r_id='.$val['id'].'&fav_p='.$currentPageNum ?>" class="box-review_records">
  ...
  <?php } ?>
    ...
    <?php if(empty($ReviewData['data'])){ ?>
          <p class="msg-none"><i class="far fa-frown-open"></i>登録はありません</p>
        <?php } ?>
